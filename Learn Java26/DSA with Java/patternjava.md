...
### let’s make pyramids with stars in Java super beginner‑friendly. A pyramid is basically a triangle that’s centered, so stars grow row by row with spaces on both sides. You can style it in many ways: filled, hollow, inverse (upside‑down), mirrored, etc. I’ll show you each with code and the output.
    ...
    * * * 
    ...
    Stars → grow or shrink depending on the loop.

    Conditions → make hollow shapes by printing spaces inside.

    By combining loops + spaces + stars, you can design any pattern: pyramid, inverse, mirror, diamond, hollow, etc.